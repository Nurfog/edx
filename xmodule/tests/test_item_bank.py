...
class ItemBankTestMixin(MixedSplitTestCase):
    ...
    def setUp(self):
        ...
        self.item_bank = BlockFactory.create(
            category="itembank", parent=self.vertical, max_count=1, display_name="My Item Bank"
        )
        self.items = [
            BlockFactory.create(category="problem", parent=self.item_bank, display_name=f"Item {i}")
            for i in range(4)
        ]
...
class ItemBankTestsMore(ItemBankTestMixin):
    ...
    def test_validation_of_matching_blocks(self):
        ...
        self.item_bank.capa_type = ANY_CAPA_TYPE_VALUE
        self._sync_lc_block_from_library()
        self.item_bank.max_count = -1
        assert self.item_bank.validate()
    ...
    def test_non_editable_settings(self):
        ...
        """
        non_editable_metadata_fields = self.item_bank.non_editable_metadata_fields
